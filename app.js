...
function toggleLessonCompletion() {
  ...
  const lesson = appData.curriculum.lessons.find(l => l.id === activeLessonId);
  if (lesson) {
    lesson.completed = !lesson.completed;
    saveState();
    renderSchedule();
    openLessonModal(activeLessonId);
  }
}
...
function renderDiary() {
  ...
  if (dates.length === 0) {
    hist.innerHTML += "<p style='color:var(--text-secondary); font-size:0.85rem;'>No sessions recorded yet.</p>";
    return;
  }
...
function renderProgress() {
  ...
  appData.curriculum.subjects.forEach(sub => {
    const subLessons = allLessons.filter(l => l.subject.toLowerCase().includes(sub.name.slice(0, 4).toLowerCase()));
    const sTotal = subLessons.length;
    const sDone = subLessons.filter(l => l.completed).length;
    ...
    row.innerHTML = `
      <div style="display:flex; justify-content:space-between; font-size:0.85rem; font-weight:600;">
        <span>${sub.name}</span>
        <span>${sPct}% (${sDone}/${sTotal})</span>
      </div>
      <div class="progress-bar-wrap">
        <div class="progress-bar-fill" style="width: ${sPct}%; background:${sPct === 100 ? "var(--success)" : "var(--accent)"};"></div>
      </div>
    `;
